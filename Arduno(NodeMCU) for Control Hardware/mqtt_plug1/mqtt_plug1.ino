...
void check_function(String Newtopic,String command){
    ...
    if(command == "Plug/1/true"){ // Plug_1_true
      Serial.print("Plug 1 is on!\n");
      delay(500);
      digitalWrite(12, LOW);
      client.publish("Thing_Plug/1/Confirm","true");
      }   
    else if(command == "Plug/1/false"){ // Plug_1_false
      Serial.print("Plug 1 is off!\n");
      delay(500);
      digitalWrite(12, HIGH);
      client.publish("Thing_Plug/1/Confirm","false");
      } // end if&else fan_acc
}
...
void loop() {
    ...
    if(count==0){
      client.publish("Message_from_thing","Plug 1 ready!");
      count++;
      }
    client.loop(); 
    digitalWrite(14,HIGH);
