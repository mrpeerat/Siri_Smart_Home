...
void check_function(String Newtopic,String command){
    ...
    if(command == "Bulb/1/true"){ // Blub_1_true
      Serial.print("Bulb 1 is on!\n");
      delay(500);
      digitalWrite(12, LOW);
      client.publish("Thing_Bulb/1/Confirm","true");
      }   
    else if(command == "Bulb/1/false"){ // Blub_1_false
      Serial.print("Bulb 1 is off!\n");
      delay(500);
      digitalWrite(12, HIGH);
      client.publish("Thing_Bulb/1/Confirm","false");
      } // end if&else fan_acc
}
...
void loop() {
    ...
    if(count==0){
      client.publish("Message_from_thing","Blub 1 ready!");
      count++;
      }
