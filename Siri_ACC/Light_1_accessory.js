...
client.on('connect', function () {
  client.subscribe('Thing_Bulb/1/Confirm')
  //client.publish('pinghome_siri', 'Bathroom Light is Ready !')
})
  ...
  if(mes === 'true'){
   ...
   //turn off the light
    ...
    lightAccessory
       ...
       .getCharacteristic(Characteristic.On)
       .updateValue(false);
  }//end else if turn off the light
}) // end call back mqtt function


var LightController = {
  name: "Ligth_1", //name of accessory
  pincode: "031-45-154",
  username: "PB:3E:EF:5B:1J:1J", // MAC like address used by HomeKit to differentiate accessories. 
  ...
  setPower: function(status) { //set power of accessory
    ...
    this.power = status;
    //client.publish('c_bathroom_acc', status ?'light bathroom on!':'light bathroom off!')
    client.publish('Thing_Bulb/1/Order', status ?'Bulb/1/true':'Bulb/1/false')
    console.log("send Bulb 1 from siri")
  ...
  setBrightness: function(brightness) { //set brightness
   ...
   // client.publish('c_bathroom_acc', 'Set Bightness to '+brightness)
